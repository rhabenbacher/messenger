...
import { Messages, login } from './data/data';
...
export class App extends Component {
  ...
  _loadMessages(loadMore) {
    ...
    if (!loadMore) { this.setState({ messages: [] }) }
  ...
  render() {
    ...
    const appState = {messages:this.state.messages,username:this.state.username,loadMore:this.state.loadMore,loading:this.state.loading,loggedIn:this.state.loggedIn};
    return (
      <View style={{ flex: 1 }} >
        ...
        <MainScreen appState={ appState } onClose={this._logout.bind(this)} onReload={() => this._loadMessages()} onLoadMore={() => this._loadMoreMessages()} />
      </View>
    );
  }
}
